...
# parse string for time units and use int() and float() handle hours/minutes and seconds/milliseconds repsectively
...
with io.open(sys.stdin.fileno(),'r',encoding='latin-1') as file_read:
    ...
    for row_idx, row in enumerate(reader):
        try:
            ...
            try:
                ...
                eprint("Value Error on Row {0}. Invalid value in Timestamp.".format(row_idx))
                continue
            ...
            try:
                ...
                eprint("Value Error on Row {0}. Invalid value in Zipcode.".format(row_idx))
                continue
            ...
            try:
                ...
                eprint("Value Error on Row {0}. Invalid value in one of FooDuration or BarDuration.".format(row_idx))
                continue
            ...
            eprint("Unicode Decoding Error on Row {0}. Invalid unicode in {1}.".format(row_idx + 2, header[col_idx]))
        except IndexError as e:
            eprint("Something wrong with CSV file. Check that it is in valid CSV format.")
            break
